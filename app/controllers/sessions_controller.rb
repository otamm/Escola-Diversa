class SessionsController < ApplicationController
  ...
  def create # the session params sent to the browser are 'params[:session] == {email: "xxx@xxx.xxx", password: "<bla bla bla>"}', meaning that the value for :session key is another hash.
    user = User.find_by(username: params[:session][:username_log].downcase) #downcase ensures a match if the address is indeed valid.
    ...
    flash.now[:danger] = "Wops! The email #{params[:session][:username]} is not registered."
    redirect_back_or :back
    end
  end
